  ...
  factory University.fromJson(Map<String, dynamic> json) {
    return University(
      ...
      website: json['web_pages']
          [0], // Ambil situs web pertama dari array dalam JSON
    );
  }
}
...
class MyAppState extends State<MyApp> {
  late Future<UniversitiesList>
      futureUniversitiesList; // Future untuk menyimpan hasil pengambilan data universitas
  ...
  Future<UniversitiesList> fetchData() async {
    ...
    if (response.statusCode == 200) {
      ...
      return UniversitiesList.fromJson(jsonDecode(
          response.body)); // Parse respons JSON dan buat objek UniversitiesList
    } else {
      // Jika respons gagal,
  ...
  void initState() {
    super.initState();
    futureUniversitiesList =
        fetchData(); // Mulai pengambilan data universitas saat initState dipanggil
  }
            ...
            builder: (context, snapshot) {
              if (snapshot.hasData) {
                ...
                return ListView.builder(
                  itemCount: snapshot
                      .data!.universities.length, // Jumlah item dalam ListView
                  itemBuilder: (context, index) {
                    return Card(
                      // Widget Card untuk setiap item universitas
                      elevation: 3, // Tingkat elevasi bayangan
                      margin: EdgeInsets.symmetric(
                          horizontal: 10, vertical: 5), // Margin dari Card
                      child: ListTile(
                        // Widget ListTile sebagai isi dari Card
                        title: Text(snapshot.data!.universities[index]
                            .name), // Judul universitas
                        subtitle: Text(snapshot.data!.universities[index]
                            .website), // Subjudul situs web universitas
                      ),
                    );
                  },
                );
